...
TEST(AddToTree_AddToEmptyTree_notCrush, asd) {
    node_t *tree = NULL;
...
TEST(AddToTree_AddNotToEmptyTree_notCrush, asd) {
    node_t *tree = Create1NodeTree();
// äîáàâëÿåì óæå ñóùåñòâóþùèé è ïðîâåðèì ÷òî куы=афдыуж
    int res = AddToTree(&tree, tree, 2);
...
TEST(FindKLowerTree_emptyTree_notCrush, asdaa) {
...
TEST(FindKLowerTree_1NodeTree_notCrush, lasjdfh) {
...
TEST(FindKLowerTree_2NodeLeft_notCrush, zmxbcnnn) {
...
TEST(FindKLowerTree_2NodeRight_notCrush, aksdghbcmxzn) {
...
TEST(FindKLowerTree_3Node_notCrush, xzcnvmb) {
    int k = 1;
...
TEST(ClearTree_AllVariants_notCrush, ksbcxncz) {
    node_t *tree = NULL;
    ClearTree(&tree);
